...
//       gHaEvtHandlers->Add (new MollerPolScalerEvtHandler("Left","HA scaler event type 140"));
//
//   To enable debugging you may try this in the setup script
//
//     MollerPolScalerEvtHandler *lscaler = new MollerPolScalerEvtHandler("Left","HA scaler event type 140");
...
Int_t MollerPolScalerEvtHandler::End( THaRunBase* )
{
  if( fScalerTree )
    fScalerTree->Write();
  return 0;
...
Int_t MollerPolScalerEvtHandler::Analyze(THaEvData *evdata)
{
  ...
  if( fDebugFile ) {
    ...
    *fDebugFile << "\nEnter MollerPolScalerEvtHandler  for fName = " << fName << endl;
    EvDump(evdata);
  }
  ...
  if( ndata >= MAXTEVT ) {
    cout << "MollerPolScalerEvtHandler:: ERROR: Event length crazy " << endl;
    ndata = MAXTEVT - 1;
  }
  ...
  for( size_t i = 0; i < scalerloc.size(); i++ ) {
    ...
    if( idx < scalers.size() && ichan < MAXCHAN ) {
      ...
      cout << "MollerPolScalerEvtHandler:: ERROR:: incorrect index " << i
           << "  " << idx << "  " << ichan << endl;
    }
  }
...
void MollerPolScalerEvtHandler::ParseVariable( const vector<string>& dbline )
{
  string sdesc;
...
void MollerPolScalerEvtHandler::ParseMap( const char* cbuf, const vector<string>& dbline )
{
  ...
  if( fNormSlot != inorm )
    cout << "MollerPolScalerEvtHandler:: WARN: contradictory norm slot " << inorm << endl;
...
// fNormSlot is the slot#, checked for consistency
    if( clkchan != kMaxUInt ) {
      ...
      fNormIdx = idx;
      if( islot != fNormSlot )
        cout << "MollerPolScalerEvtHandler:: WARN: contradictory norm slot ! "
             << islot << endl;
      if( fDebugFile )
...
void MollerPolScalerEvtHandler::AssignNormScaler()
{
...
void MollerPolScalerEvtHandler::VerifySlots()
{
  for( size_t i1 = 0; i1 < scalers.size() - 1; i1++ ) {
    for( size_t i2 = i1 + 1; i2 < scalers.size(); i2++ ) {
      if( scalers[i1]->GetSlot() == scalers[i2]->GetSlot() )
        cout << "MollerPolScalerEvtHandler:: WARN:  same slot defined twice" << endl;
    }
  }
}

void MollerPolScalerEvtHandler::SetIndices()
{
...
THaAnalysisObject::EStatus MollerPolScalerEvtHandler::Init(const TDatime& date)
{
  const int LEN = 200;
  char cbuf[LEN];
  ...
  // cout << "Howdy !  We are initializing MollerPolScalerEvtHandler !!   name =   "
  //      << fName << endl;
...
// Parse the map file which defines what scalers exist and the global variables.
  ...
  if(fDebugFile) {
    *fDebugFile << "MollerPolScalerEvtHandler:: Name of scaler bank "<<fName<<endl;
    for (size_t i=0; i<scalers.size(); i++) {
...
void MollerPolScalerEvtHandler::AddVars( const TString& name, const TString& desc,
...
void MollerPolScalerEvtHandler::DefVars()
{
  ...
  if( gHaVars ) {
    if( fDebugFile )
      *fDebugFile << "MollerPolScalerEvtHandler:: Have gHaVars " << gHaVars << endl;
  } else {
    cout << "No gHaVars ?!  Well, that's a problem !!" << endl;
    return;
  }
  if( fDebugFile )
    *fDebugFile << "MollerPolScalerEvtHandler:: scalerloc size " << scalerloc.size() << endl;
...
ClassImp(MollerPolScalerEvtHandler)
