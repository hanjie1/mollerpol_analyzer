...
void MOLLERGEMModule::find_clusters_1D_experimental( MOLLERGEM::GEMaxis_t axis, Double_t constraint_center, Double_t constraint_width ){
  ...
  std::map<UShort_t,std::map<UShort_t,mollergemcluster_t> > protoclusters; //clustering results in individual time samples
  ...
  //We can use the mollergemcluster_t structure for the protoclusters because
  //they have all the basic variables we need (and some that won't make sense until we merge the time samples together):
  //std::vector<std::vector<mollergemcluster_t> > protoclusters(fN_MPD_TIME_SAMP);
  ...
  for( auto stripmax : allmaxima ){
    if( Nsamp_allmaxima[stripmax] > 2 ){ // we want this strip to be a local max in at least three time samples (at this stage we aren't requiring them to be consecutive)
      ...
      mollergemcluster_t fullcluster;
	...
	if( TESTBIT( timesamples_allmaxima[stripmax], isamp ) ){ //then this strip is a local maximum in this time sample:
	  mollergemcluster_t &protocluster = protoclusters[stripmax][isamp];
	  if( ngoodsamp == 0 ){
...
void MOLLERGEMModule::fill_2D_hit_arrays(){
  ...
  for( UInt_t iu=0; iu<fNclustU; iu++ ){
	...
	mollergemhit_t hittemp; // declare a temporary "hit" object:

	//copying overhead, might be inefficient:
	//mollergemcluster_t uclusttemp = fUclusters[iu];
	//mollergemcluster_t vclusttemp = fVclusters[iv];
